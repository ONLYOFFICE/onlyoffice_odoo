...
QUnit.module("web_widget_dropdown_dynamic", (hooks) => {
  ...
  QUnit.test("values are fetched with changing context", async function (assert) {
    ...
    await editInput(target, ".o_field_widget[name='change_field'] input", "step-other")

    assert.containsN(target, "option", 1)
    assert.verifySteps(["get_views", "web_read", "method_name", "method_name", "method_name", "method_name"])
  })
  QUnit.test("values are fetched w/o context (char)", async (assert) => {
    ...
    assert.containsOnce(field_target, "option[value='\"value b\"']", "got `value b` ")
  })
  ...
  QUnit.test("displays label for selected value in readonly mode", async (assert) => {
    ...
    assert.ok(fieldEl.querySelector("span"), "field renders a <span> element in readonly mode")
    assert.strictEqual(
