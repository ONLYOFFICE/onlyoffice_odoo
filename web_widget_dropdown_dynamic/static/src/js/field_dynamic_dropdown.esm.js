...
export class FieldDynamicDropdown extends Component {
  ...
  async _fetchSpecialData(props) {
    ...
    if (!specialDataCaches[key]) {
      specialDataCaches[key] = await orm.call(resModel, props.method, [], { context: props.context })
    }
  ...
  get string() {
    ...
    const option = this.specialData.find((o) => o[0] === value || String(o[0]) === String(value))
  ...
  get options() {
    ...
    if (["char", "integer", "selection"].includes(fieldType)) {
      if (
        ...
        !this.specialData.map((val) => val[0]).includes(String(this.props.record.data[this.props.name]))
      ) {
